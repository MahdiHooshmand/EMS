...
# Device appearance for BLE advertising
...
async def handle_auth_request(connection):
    ...
    while True:
        ...
        received_username, received_password = await get_credentials()
        ...
        if is_auth:
            ...
            send_token(tk, connection)

            # Check if response after sending token is 'OK'
            if await check_response_after_auth(connection):
                return un, pw, token
            else:
